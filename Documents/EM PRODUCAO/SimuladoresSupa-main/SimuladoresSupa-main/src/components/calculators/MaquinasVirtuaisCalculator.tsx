...
const MaquinasVirtuaisCalculator = ({ onBackToDashboard }: MaquinasVirtuaisCalculatorProps) => {
    ...
    // Função para lidar com mudança de regime tributário
    const handleTaxRegimeChange = (regime: string) => {
        setSelectedTaxRegime(regime);

        // Definir valores padrão baseados no regime selecionado
        switch (regime) {
            case 'simples_nacional':
                setPisCofins('15,00'); // Simples Nacional com PIS/COFINS 15%
                setCsllIr('0,00');     // CSLL 0%
                setIss('0,00');
                break;
            case 'lucro_real':
                setPisCofins('9,25');
    ...
    const revenueTaxes = useMemo(() => {
        const total = (taxRates.simplesNacional || 15) / 100;
        return total;
                                                        ...
                                                        <Table>
                                                            ...
                                                            <TableBody>
                                                                ...
                                                                <TableRow className="border-slate-800">
                                                                    <TableCell className="text-white pl-8">Simples Nacional ({formatBrazilianNumber(taxRates.simplesNacional)}%)</TableCell>
                                                                    <TableCell className="text-right text-white">{formatCurrency((costBreakdown.grossRevenue || costBreakdown.finalPrice) * (taxRates.simplesNacional / 100))}</TableCell>
                                                                </TableRow>
                                                                <TableRow className="border-slate-800">
                                                                    <TableCell className="text-white pl-8">Custo/Desp ({formatBrazilianNumber(taxRates.custoDesp)}%)</TableCell>
                                                                    <TableCell className="text-right text-white">{formatCurrency((costBreakdown.grossRevenue || costBreakdown.finalPrice) * (taxRates.custoDesp / 100))}</TableCell>
                                                                </TableRow>

...
const InternetManCalculator: React.FC<InternetManCalculatorProps> = ({ onBackToDashboard }) => {
    ...
    // Função para mudança de regime tributário
    ...
    // useEffect para carregar configurações padrão
    useEffect(() => {
        // Carregar configurações salvas ou usar padrões
        const savedTaxRegime = localStorage.getItem('internetManTaxRegime');
        const savedTaxValues = localStorage.getItem('internetManTaxValues');

        if (savedTaxRegime && savedTaxValues) {
            setSelectedTaxRegime(savedTaxRegime);
            setTaxRegimeValues(JSON.parse(savedTaxValues));
        } else {
            // Configurações padrão: Simples Nacional
            handleTaxRegimeChange('simples_nacional');
        }
    }, []);
    ...
    // Função para calcular DRE por período de contrato com payback detalhado
    const calculateDREForPeriod = useCallback((months: number) => {
        // Receita mensal = valor mensal × número de meses do período
        let monthlyValue = 0;
        let totalRevenue = 0;
        ...
        // Custo de banda = velocidade × 2,09 × meses do período
        const velocidade = result?.speed || 0; // Velocidade em Mbps
        const custoBandaMensal = velocidade * taxRates.banda; // velocidade × 2,09
        const custoBanda = custoBandaMensal * months;

        // Custo MAN vem da calculadora conforme prazo contratual e velocidade
        const custoManCalculadora = custoMan;

        const fundraising = 0; // Conforme tabela
        const lastMile = 0; // Conforme tabela

        // Impostos baseados na receita total (incluindo taxa de instalação)
        ...
        // Payback Calculation - Cálculo detalhado mês a mês
        let paybackMonths = 0;
        let acumulado = 0;

        // Fluxo de caixa da instalação (primeira parcela à vista)
        // Taxa de instalação menos custo, imposto de 15% e taxa Custo/Desp de 10%
        const taxaInstalacaoLiquida = receitaInstalacao - custoManCalculadora - (receitaInstalacao * 0.15) - (receitaInstalacao * 0.10);
        acumulado += taxaInstalacaoLiquida;

        // Fluxo de caixa mensal líquido (para payback)
        // O período do contrato só começa após 30 dias
        ...
        return {
            receitaMensal: totalRevenue, // Receita total do período
            receitaInstalacao,
            receitaTotalPrimeiromes,
            custoMan: custoManCalculadora, // Custo MAN da calculadora
            custoBanda, // Custo de banda calculado
            fundraising,
            lastMile,
            simplesNacional,
            comissaoVendedor,
            ...
            margemLiquida,
            markup,
            paybackMonths, // Payback calculado mês a mês
            totalCost
        };
    }, [
        result,
    ...
    const dreCalculations = useMemo(() => {
        ...
        return {
            ...
            paybackMeses: dre12.paybackMonths,
        };
                                            ...
                                            <Button
                                                ...
                                                onClick={() => {
                                                    if (isEditingTaxes) {
                                                        ...
                                                        localStorage.setItem('internetManTaxRegime', selectedTaxRegime);
                                                        localStorage.setItem('internetManTaxValues', JSON.stringify(taxRegimeValues));
                                                    }
                                                    setIsEditingTaxes(!isEditingTaxes);
                                                }}
                                                className="border-slate-600 text-slate-300 hover:bg-slate-700"
                                            >
                                        ...
                                        <div className="space-y-6">
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <Select
                                                    ...
                                                    onValueChange={handleTaxRegimeChange}
                                                ...
                                                >
                                                    ...
                                                    <SelectContent>
                                                        ...
                                                        <SelectItem value="lucro_real_reduzido">Lucro Real Reduzido</SelectItem>
                                                    </SelectContent>
                                                </Select>
                                            </div>
                                            ...
                                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                                <div className="space-y-2">
                                                    <Label htmlFor="pis-cofins-rate">PIS/COFINS (%)</Label>
                                                    <Input
                                                        id="pis-cofins-rate"
                                                        type="text"
                                                        value={taxRegimeValues.pisCofins}
                                                        onChange={(e) => {
                                                            if (isEditingTaxes) {
                                                                setTaxRegimeValues(prev => ({ ...prev, pisCofins: e.target.value }));
                                                            }
                                                        }}
                                                        disabled={!isEditingTaxes}
                                                ...
                                                <div className="space-y-2">
                                                    <Label htmlFor="iss-rate">ISS (%)</Label>
                                                    <Input
                                                        id="iss-rate"
                                                        type="text"
                                                        value={taxRegimeValues.iss}
                                                        onChange={(e) => {
                                                            if (isEditingTaxes) {
                                                                setTaxRegimeValues(prev => ({ ...prev, iss: e.target.value }));
                                                            }
                                                        }}
                                                        disabled={!isEditingTaxes}
                                                        className="bg-slate-800 border-slate-700"
                                                ...
                                                <div className="space-y-2">
                                                    <Label htmlFor="csll-ir-rate">CSLL/IR (%)</Label>
                                                    <Input
                                                        id="csll-ir-rate"
                                                        type="text"
                                                        value={taxRegimeValues.csllIr}
                                                        onChange={(e) => {
                                                            if (isEditingTaxes) {
                                                                setTaxRegimeValues(prev => ({ ...prev, csllIr: e.target.value }));
                                                            }
                                                        }}
                                                        disabled={!isEditingTaxes}
                                                        className="bg-slate-800 border-slate-700"
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                <div className="space-y-2">
                                                    <Label htmlFor="banda-rate">Banda (R$/Mbps)</Label>
                                                    <Input
                                                        id="banda-rate"
                                                        type="number"
                                                        step="0.01"
